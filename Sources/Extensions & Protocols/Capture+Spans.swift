...
extension Capture.Logger {
    ...
    static func endSpan(_ span: GameSpan, result: SpanResult) {
        self.spanMap[span]?.end(result)
        self.spanMap[span] = nil
    }
    ...
    static func startSpan(_ span: GameSpan, parent: GameSpan? = nil, level: LogLevel = .debug) {
        ...
        let parentID = parent.flatMap { self.spanMap[$0]?.id }
        self.spanMap[span] = Self.startSpan(name: span.rawValue, level: level, parentSpanID: parentID)
    }
}
